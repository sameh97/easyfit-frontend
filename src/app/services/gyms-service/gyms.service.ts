...
export class GymsService {
  ...
  public getAll = (): Observable<Gym[]> => {
    return this.http.get<Gym[]>(`${this.url}/gym`, {
      headers: CoreUtil.createAuthorizationHeader(),
    });
  };

  public create = (gym: Gym): Observable<any> => {
    return this.http.post<Gym>(`${this.url}/add-gym`, gym, {
      headers: CoreUtil.createAuthorizationHeader(),
    });
  };

  public update = (gym: Gym): Observable<Gym> => {
    return this.http.put<Gym>(`${this.url}/gym`, gym, {
      headers: CoreUtil.createAuthorizationHeader(),
    });
  };

  public delete = (id: number): Observable<any> => {
    return this.http.delete(`${this.url}/gym?id=${id}`, {
      headers: CoreUtil.createAuthorizationHeader(),
    });
  };
}
